#importing random library for randomising objects wtihin the game
import random
...
#function defining player's attack mechanism
    def attack(self):
        ...
        if self.health2 < 1:
            ...
            print('\n' * 2)
            event1.event1()#calls function event1
...
# this section is for the first decision selection
    def event1(self):
            ...
            decision = decision.lower()#lowers user input to avoid errors
            print('\n' * 80)

            if decision == "fight till death" or decision == "fight":
                self.health2 = 100
...
#this function specifies event 2 out of 3
#it is split into two different decision a player can make
#this section is for the first decision selection
    def event2(self):
        decision = input("....FOLLOWED BY A LARGE PATROL NEAR THE BRIDGE! HIDE IN THE RIVER OR USE THE BOAT: ")
        decision = decision.lower()
        if decision == "hide in the river" or decision == "hide":
            print('\n' * 1)
            hide1 = input("PRESS F TO HOLD YOUR BREATH UNTIL THEY HAVE PASSED: ")
            hide1 = hide1.lower()
            if hide1 == "f":
                print('\n' * 1)
                print("NOW FOLLOWING THEIR TRAIL", player, "IS STEALTHY HEADING TO THE OLD BAZAR WHERE THE CURRENT OLD TOWN LEADER IS GIVING HIS SPEECH,"
            ...
            if item == "f":
                ...
                print('\n' * 1)
                assassination.assassination()#calls assassination function



#this section is for the second decision selection
        elif decision == "use the boat" or decision == "boat":
            print('\n' * 1)
            hide2 = input("PRESS E TO USE THE BOAT TO GET AWAY: ")
            hide2 = hide2.lower()
            if hide2 == "e":
                print("NOW USING THE BOAT UNDER THE CLOAK OF FOG", player, "IS HEADING TO THE OLD BAZAR WHERE THE CURRENT OLD TOWN LEADER IS GIVING HIS SPEECH PERFECT TIME FOR ASSASSINATION")
            print('\n' * 2)
            print("***30 MIN LATER***")
            print('\n' * 2)
...
#this function is being called in the event2 function
#section section below is for assassination a
    def assassination(self):
        assasination = input("UPON DISCOVERING THE AREA, PLAN YOUR ASSASSINATION: 'a'for bow shot or 'b' for smoke bomb kill: ")
        assasination = assasination.lower()
        if assasination == "a":
            print('\n' * 1)
            input("YOU ARE NOW ONE SHOT AWAY FROM TAKING THE LEAD OF THE OLD TOWN, PRESS ENTER TO USE THE PRECISION KILL: ")
            print('\n' * 2)
            print("***HEADSHOT***")
            print('\n' * 2)
...
#this event is split into sequential events with no player choice
    def event3(self):
        print("***",player, ",YOU ARE NOW IN NO MAN'S LAND WHICH HAVE NOT BEEN CONQURED BEFORE BY ANY WARRIOR***")
        print('\n' * 2)
        ...
        print("AS", player,"APPROACHES THE UNHOLY TEMPLE TO CONFRONT THE FINAL ENEMY IN HIS WAY, HE IS ATTACKED BY THE CARETAKER, THE RELIC MONSTER THE SERVANT OF GODS")
        print('\n' * 2)
                ...
                if bomb == "b":
                    ...
                    if attack == "h":
                        ...
                        if attack2 == "r":
                            ...
                            print('\n' * 1)
                            final_strike = input("PRESS V TO USE YOUR BOW TO EXPLODE TEMPLE PILLARS WHICH WILL CRUSH THE ENEMY ENSURING OUR VICTORY!!: ")
...
#function to display player's current progress
    def location3(self):
        ...
        if location == 3:
            ...
            event3.event3()#calls event3 function



#function map being called to print the map at the start of the game
print(map())
...
#and statement to randomise an enemy and a weapon type
...
#calls foe.attack function to allow the player to select his attack option,
#and main.damage function to allow the player to take damage
while foe.alive == True:
    kill = input("\n press Enter to start an attack!")
    if kill != "":
        print ("\n*PRESS THE CORRECT ACTION BUTTON*")
    print('\n' * 2)
    if kill == "":
        foe.attack()
        main.damage()
